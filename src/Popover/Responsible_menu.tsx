...
export default function Responsible_menu() {

    const [anchorEl, setAnchorEl] = React.useState<HTMLButtonElement | null>(null);
    const dispatch = useDispatch<any>();
    const username = useSelector(selectuserData);
    const [data, setData] = useState<ItemType[] | null>(null);
    const [Responsible, setSelectedResponsible] = React.useState<string>('');
    ...
    const handleChangeResponsible = (event: SelectChangeEvent) => {
        setSelectedResponsible(event.target.value as string);
    };
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (isConfirmed) {
                ...
                const response = await axios.post(`${import.meta.env.VITE_APP_API_BASE_URL}/api/Responsible/create-Responsible`, {
                    ...formData,
            ...
            Swal.fire({
                ...
                text: 'มีข้อผิดพลาดในการเพิ่มผู้ผลิต',
            });
        } finally {
            handleClose();
        }
        ...
        <div>
            ...
            >
                <div className='flex justify-between items-center ml-4 mr-4 mt-2 mb-2'>
                    <div>เพิ่มรายชื่อผู้รับผิดชอบ</div>
                ...
                <Paper sx={{ width: { xs: '340px', xl: '1200px' }, padding: '40px' }}>
                    <Grid container spacing={2}>
                        <Container maxWidth="sm">
                            <TextField label="ชื่อ-นามสกุล" variant="outlined" fullWidth sx={{ marginTop: '10px' }} onChange={(e) => handleInputChange('responsible_name', e.target.value)} />
                            <TextField label="รหัสพนักงาน" variant="outlined" fullWidth sx={{ marginTop: '10px' }} onChange={(e) => handleInputChange('employee_code', e.target.value)} />
